...
namespace Documents.Moduls
{
     static class ApiWork
     {
        ...
        public static void UpdateUser(string newEmail,string newFullName, string newRole) 
        {
            var user = new User(newEmail,newFullName,newRole);
            var response = await $"{baseUrl}".AppendPathSegment("/update_user").PutJsonAsync(user);
        }

        public static void UpdateDocument(string newName,string newDescription, Template newTemplate, byte newFile)
        {
        ...
        public static void UpdateRole(string name, int count)
        {
            var role = new Role(name, count);
            var response = await $"{baseUrl}".AppendPathSegment("/update_role").PutJsonAsync(role);
        }

        public static void CreateRole(string name,int count)
        {
            var role = new Role(name, count);
            var response = await $"{baseUrl}".AppendPathSegment("/create_role").PostJsonAsync(role);
        }

        public static void CreateDocument(string name, string description, Template template, byte file)
        {
        ...
        public static void CreateTemplate(string name, int count)
        {
